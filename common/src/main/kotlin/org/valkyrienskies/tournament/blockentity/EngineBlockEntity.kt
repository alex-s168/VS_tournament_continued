...
import org.valkyrienskies.Tournament.TournamentProperties.HEAT
...
import org.valkyrienskies.Tournament.util.KtContainerData
import org.valkyrienskies.mod.common.getShipManagingPos
import kotlin.math.ceil

class EngineBlockEntity(pos: BlockPos, state: BlockState) :
    BaseContainerBlockEntity(TournamentBlockEntities.ENGINE.get(), pos, state),
    ServerShipProvider,
    StackedContentsCompatible,
    WorldlyContainer {

    override val ship: ServerShip? get() = (this.level as ServerLevel).getShipManagingPos(this.blockPos)
    private val TournamentShipControl by shipValue<TournamentShipControl>()
    ...
    override fun getDefaultName(): Component = TranslatableComponent("gui.vs_tournament.engine")
    ...
    fun tick() {
        if (!this.level!!.isClientSide) {
            ...
            if (this.fuelLeft > 0) {
                ...
                if (this.heat < 100f) {
                    this.heat += TournamentConfig.SERVER.engineHeatGain
                }
            ...
            if (heat > 0 && ship != null && TournamentShipControl != null) {
                TournamentShipControl!!.power += lerp(
                    heat / 100f,
                    TournamentConfig.SERVER.minEnginePower,
                    TournamentConfig.SERVER.enginePower
                )

                heat -= TournamentShipControl!!.consumed
            }

            if (heat > 0) {
                heat -= min(TournamentConfig.SERVER.engineHeatLoss, heat)
            }
        }
    }
